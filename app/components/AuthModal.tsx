...
import { registrarClienteManual, loginClienteManual } from "@/firebase/clientes";
...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
    ...
    const [manualClient, setManualClient] = useState<any>(null);

    useEffect(() => {
        // Cargar sesión manual si existe
        const savedClient = localStorage.getItem("cliente_manual");
        if (savedClient) {
            setManualClient(JSON.parse(savedClient));
        }
    ...
    const handleEmailAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (isLogin) {
                // Inicio de sesión manual para clientes
                const result = await loginClienteManual(email, password);
                if (result.success) {
                    localStorage.setItem("cliente_manual", JSON.stringify(result.cliente));
                    setManualClient(result.cliente);
                    Swal.fire({
                        icon: 'success',
                        title: '¡Bienvenido!',
                ...
                // Registro manual para clientes
                const result = await registrarClienteManual({
                ...
                if (result.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Cuenta creada',
                        text: 'Te hemos registrado con éxito en nuestra base de datos.',
                        background: '#0a0a0a',
                        color: '#fff',
                        confirmButtonColor: '#fbbf24'
                    });
    ...
    const handleLogout = async () => {
        try {
            if (manualClient) {
                localStorage.removeItem("cliente_manual");
    ...
    const activeUser = user || manualClient;
            ...
            <div className="relative w-full max-w-md bg-pedal-bgMain border border-pedal-primary-glow/20 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                ...
                <div className="p-6 flex items-center justify-between border-b border-white/5">
                    <h2 className="text-xl font-bold text-white">
                        {activeUser ? "Mi Cuenta" : (isLogin ? "Iniciar Sesión" : "Crear Cuenta")}
                    </h2>
                    <button 
                        onClick={onClose}
                    ...
                    {activeUser ? (
                        <div className="flex flex-col gap-6 text-center">
                            <div className="flex flex-col items-center gap-3">
                                {activeUser.photoURL ? (
                                    <img src={activeUser.photoURL} alt="Profile" className="w-20 h-20 rounded-full border-2 border-pedal-primary-glow/50" />
                                ) : (
                                ...
                                <div>
                                    <p className="text-white font-semibold text-lg">{activeUser.displayName || activeUser.nombre || "Usuario"}</p>
                                    <p className="text-white/50 text-sm">{activeUser.email}</p>
                                </div>
                            </div>
